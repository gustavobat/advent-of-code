...
fn parse_paths(path: std::path::PathBuf) -> Result<Vec<Vec<(usize, usize)>>> {
...
fn main() -> Result<()> {
    let mut paths = parse_paths(std::path::PathBuf::from("./data/14.input"))?;
...
mod day14 {
    ...
    #[test]
    fn part1() -> Result<()> {
        let mut paths = parse_paths(std::path::PathBuf::from("./data/14.test"))?;
        let (mut grid_with_abyss, source_x) = make_grid(&mut paths, false);
        let res = solve(&mut grid_with_abyss, source_x);
        assert_eq!(res, 24);
        Ok(())
